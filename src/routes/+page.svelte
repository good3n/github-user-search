...
<div class="container pt-24 pb-48 relative z-10">
	<svg class="w-36 fill-white block mx-auto" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
		<path
	...
	<h1 class="text-3xl text-center mt-8">GitHub User Search</h1>
	...
	<div>
		{#each users as { id, avatar_url, bio, blog, company, created_at, html_url, login, name, public_repos, followers, following, location, twitter_username } (id)}
			<div class="bg-slate-800 rounded-xl mt-6 p-14 flex gap-x-12">
				<img class="h-32 w-32 rounded-full" src={avatar_url} alt={name} />
				<div class="w-full">
					<header class="grid grid-cols-[2fr_1fr] items-center">
						...
						<p class="col-span-2 mt-8 text-slate-400">
							{#if bio}
								{bio}
							{:else}
					...
					<span
						class="block bg-slate-900 rounded-xl flex items-center justify-between py-6 px-12 mt-8"
					>
						<p>
					...
					<footer class="mt-8 grid grid-cols-2 gap-x-4 gap-y-4 text-slate-300">
						{#if location}
